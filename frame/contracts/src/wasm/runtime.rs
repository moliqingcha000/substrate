...
bitflags! {
	...
	struct CallFlags: u32 {
		...
		/// Abort all execution if the contract that calls `seal_call` is already on the stack.
		const DENY_REENTRANCY = 0b0000_1000;
	}
}
		...
		UncheckedFrom<<E::T as frame_system::Config>::Hash> + AsRef<[u8]>
{
	...
	fn call(
		...
		if flags.contains(CallFlags::DENY_REENTRANCY) && self.ext.is_executing(&callee) {
